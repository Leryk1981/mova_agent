...
async function main() {
  ...
  try {
    ...
    process.env.OCP_RATE_LIMIT_STRICT_OVERRIDE = '';
    ...
    const fixture = await loadFixture(
      'packs/ocp_rate_limit_throttle_v0/fixtures/pos/two_sends_soft_throttle.json',
      targetUrl
    );
    ...
    const assertions = [
      ...
      { name: 'second_throttled', passed: secondEvidence.outcome_code === 'THROTTLED' },
    ...
    if (!allPassed) {
      console.error('[quality:ocp_rate_limit_throttle] FAIL', reportDir);
      process.exit(1);
    }
    console.log('[quality:ocp_rate_limit_throttle] PASS', reportDir);
  } catch (error) {
    console.error('[quality:ocp_rate_limit_throttle] FAIL', error.message);
    process.exit(1);
  } finally {
    server.close();
  }
