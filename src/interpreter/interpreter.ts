...
import { EpisodeWriter, RunEpisodeWriter } from '../episodes/episode_writer';
...
class Interpreter {
  ...
  async runPlan(params: {
    ...
    const { planEnvelope, toolPool, instructionProfile, tokenBudgetPath, tokenBudgetProfile } =
      params;

    try {
      // Load token budget contract
      const tokenBudgetContract = await this.tokenBudgetLoader.load(
        tokenBudgetPath,
        tokenBudgetProfile
      );
      ...
      try {
        for (const step of planEnvelope.payload.steps) {
          ...
          if (budgetStatus.exceeded) {
            ...
            for (const violation of budgetStatus.violations) {
              const budgetViolationEpisode =
                context.tokenBudgetEnforcer.createBudgetViolationEpisode(violation, {
          ...
          input_data_refs: [
            {
              ...
              data_id:
                planEnvelope.payload.steps.length > 0
        ...
        context.tokenMeter.saveReport(
          path.join(evidenceDir, 'token_usage.json'),
          isQualitySuite ? 'quality suite uses noop handlers' : undefined
        );
      }
    } catch (error: any) {
    ...
    try {
      ...
      if (toolBudgetStatus.exceeded) {
        ...
        for (const violation of toolBudgetStatus.violations) {
          const budgetViolationEpisode = context.tokenBudgetEnforcer.createBudgetViolationEpisode(
            violation,
            { run_id: context.run_id, request_id: context.request_id, step_id: step.id }
          );
          await context.episodeWriter.writeSecurityEvent(budgetViolationEpisode);
        }
  ...
  private async recordSecurityEvent(
    ...
    const category:
      ...
      | 'config'
      | 'infrastructure'
      | 'other' = this.getSecurityEventCategory(eventType);
    const eventSeverity: 'info' | 'low' | 'medium' | 'high' | 'critical' = severity;
  ...
  async executePlan(plan: any): Promise<any> {
    ...
    const tokenBudgetEnforcer = new TokenBudgetEnforcer(
      { version: '1.0', limits: {}, policy: { on_budget_exceeded: 'warn' } },
      tokenMeter
    );
...
export { Interpreter, PlanStep, PlanEnvelope, ToolPool, InstructionProfile };
